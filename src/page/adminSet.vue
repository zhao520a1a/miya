...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            ...mapState({
               adminInfo:
                   function(state){//箭头函数会有this的问题
                        let localData = window.localStorage.getItem('adminInfo')
                        if(state.adminInfo.id===-1 && localData){
                            this.$store.commit('saveAdminInfo',localData)//同步操作
                        }
